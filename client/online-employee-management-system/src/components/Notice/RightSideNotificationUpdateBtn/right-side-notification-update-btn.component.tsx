...
export default function RightSideNotificationUpdateBtn(): JSX.Element {
  ...
  const [isNotificationGranted, setIsNotificationGranted] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    isNotificationOn && accessNotification();
  }, [isNotificationOn])
  ...
  return (
    <div className={`${styles.mainContainer} flex items-center`}>
      ...
      {!isNotificationOn ? <div className="w-60 text-xl">
        {isNotificationGranted === false ? "Notification permission denied. Allow notifications for latest updates." : "Turn on notifications for latest updates."}

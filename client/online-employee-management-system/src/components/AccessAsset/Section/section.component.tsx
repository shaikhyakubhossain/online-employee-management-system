...
export default function Section(props: propsType): JSX.Element {
  const placeholderCount = 10; // Show 10 placeholders to prevent layout shift
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {!props.data ? (
          Array.from({ length: placeholderCount }).map((_, index) => (
            <div
              key={index}
              className="w-[280px] h-[320px] mx-auto flex items-center justify-center border border-gray-300 rounded-lg bg-white text-blue-600"
            >
              Loading...
            </div>
          ))

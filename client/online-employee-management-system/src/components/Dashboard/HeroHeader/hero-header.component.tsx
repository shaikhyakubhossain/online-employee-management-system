...
export default function HeroHeader(props: propsType): JSX.Element {
  return (
    <div
      className={`${styles.mainContainer} flex justify-between ${
        props.swapTextLocation ? "flex-row-reverse" : ""
      } min-h-36 max-h-80 mb-7 pl-8 pr-24 rounded-2xl shadow-md`}
    >
      <div className={`${styles.left} my-auto`}>
        <Greeting
          ...
          showCurrentUserName={props.showCurrentUserName}
        />
      </div>
        ...
        {props.showSvg && (
          <div
            className={`${styles.svgs} ${
              props.swapTextLocation ? "-left-14" : "-left-24"
            } -top-14`}
          >
            <img src={svgForHeroHeader[0].base64} alt="" />
          </div>
        )}
        <div className="font-bold text-6xl my-auto welcome">
          {props.greeting}
        </div>
        {props.showSvg && (
